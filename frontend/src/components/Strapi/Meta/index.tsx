...
const StrapiMeta = ({ seo }: MetaProps) => {
  ...
  const baseURL = process.env.NODE_ENV === 'production' 
	...
	const fullURL = seo?.slug ? `${baseURL}/${seo.slug}`.replace(/\/\/+/g, '/') : undefined;
  ...
  return (
    <Head>
      ...
      {seo?.canonicalUrl && <link rel="canonical" href={seo.canonicalUrl} />}
			...
			{renderMetaTag({ property: "description", name: "description", content: seo?.metaDescription})}
			{renderMetaTag({ property: "og:description", content: seo?.metaDescription })}
			{renderMetaTag({ property: "og:type", content: seo?.metaType })}
			{renderMetaTag({ property: "og:url", content: fullURL })}
			{renderMetaTag({ property: "og:image:alt", content: seo?.metaImage?.image?.data?.attributes?.alternativeText })}
			{renderMetaTag({ property: "og:image", content: `${imageSrc}?${ms}` })}
			{renderMetaTag({ property: "twitter:image", content: `${imageSrc}?${ms}` })}
