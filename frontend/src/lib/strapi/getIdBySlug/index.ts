...
const strapiGetIdBySlug = ({
  collection,
  slug,
  slugPath,
  fullPath = true,
}: StrapiGetIdBySlugParams): string | undefined => {
  if (!arrayIsValid(collection)) {
    throw new Error(
      `Error in strapiGetIdBySlug: collection must be a valid array. Found ${collection}`,
    );
  }
  if (!slug) {
    throw new Error(
      `Error in strapiGetIdBySlug: slug must be provided. Found ${slug}`,
    );
  }
  ...
  const matchedItem = collection.find(
    (item) => getObjectKeyByString(item, slugPath) === uri,
  );
  ...
  throw new Error(
    `Error in strapiGetIdBySlug: could not locate item '${uri}' in collection`,
  );
};
